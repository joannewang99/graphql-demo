...
if [ "$WAIT_FOR_ENDPOINT" = "true" ]; then
    echo "Waiting for Hasura endpoint http://localhost:8080 to be available..."
    while true; do
        if curl -s -o /dev/null "http://localhost:8080/v1/version"; then
            echo "Endpoint http://localhost:8080 is now available."
            break
        else
            echo "Endpoint http://localhost:8080 not available, waiting..."
            sleep 1
        fi
    done
fi
